# 爆品选品雷达 · PRD 符合性审计报告（P0 修复后 · v2.7）

> **文档类型**：领导汇报 / 合规审计（修复版）  
> **审计日期**：2026-06-05（北京时间）  
> **文档版本**：**v2.7**（除真实数据外全量复检，代码基线 `4b99468`）  
> **修复基线**：P0 空文件恢复 + 构建验证  
...
| **真实 API 接入** | **不符合（刻意保留）** | `jdbc-only`；架构层已具备 `external-primary` 切换能力 |
| **路线图经营 KPI 字面达成** | **不符合** | v2.6 起为应用内推算（`demoData: false`），非外部可审计经营结果 |
| **除真实数据外的 PRD/背景功能面** | **符合（≈98%）** | 见 §九 v2.7 复检 |
| **背景 P2（供应链匹配、团队协作）** | **符合（演示级）** | `/supply-match`、`/team` + 对应 API（v2.4） |
...
| 痛点清单 | 全局静态 `InsightNarrativeContent` | ✅ `PainPointListBuilder` + `CompetitorComplaintAggregator`；v2.6 情感规则极性 |
...
| 同质化评分 | 洞察卡片表未单独列 | ✅ v2.6 洞察卡片列 + 竞争图 tooltip |
| 五大数据维度 | 数据准备无独立配置 UI | ✅ v2.6 `DataDimensionCoverage` 映射面板 |
| KPI 与工作区 | 侧栏/移动看板未随 brandId 刷新 | ✅ `4b99468` 洞察/移动看板均传 `brandId` |
| 数据 | 蝉妈妈/飞瓜/评论 API 未接真实源 | **唯一刻意保留的生产缺口** |
| 运营指标 | 静态样例 KPI | ✅ v2.6 `OperationalMetricsCalculator` 应用内推算；仍非外部财报 |
...
| v2.3 | 2026-06-05 | 雷达/TOP50/报告 summary 平台口径联动（`f51fa7e`） |
| **v2.4** | **2026-06-05** | **P0 外部市场 API 接入层；P1 看板/趋势 API 平台化 + 痛点仅差评聚合；PRD 移动端改 H5；P2 供应链匹配/团队协作** |
| **v2.5** | **2026-06-05** | **全链路 `platform` query 统一：WorkflowStepper 导航、雷达/洞察/机会/报告 workflow、推送 digest 按平台口径** |
| **v2.6** | **2026-06-05** | **非真实数据缺口补齐：应用内 KPI/核心承诺推算、品牌工作区、团队权限与审批、痛点情感规则、洞察同质化列、12 月趋势文案、五维数据面板、归因联动 action_status、模型训练流水线** |
| **v2.7** | **2026-06-05** | **除真实数据外全量复检（基线 `4b99468`）；`4b99468` 修复洞察/移动看板 KPI 随 `brandId` 联动；§九 收口判定** |

---

## 九、除真实数据外符合性复检（v2.7 · 基线 `4b99468`）

> **复检日期**：2026-06-05（北京时间）  
> **代码基线**：`4b99468`（含 v2.6 非数据缺口补齐 + KPI `brandId` 联动）
...
|------|------|
| 产品路径五步 + 竞品扩展 | ✅ 符合 |
| PRD `01/02/03` 三模块（样例 + 规则） | ✅ 符合 |
| 背景 MVP + 迭代 1 | ✅ 符合 |
| 背景迭代 2 | ⚠️ 演示级符合（归因/模型/团队/供应链/开放 API） |
| PRD 脚注「本地 MVP · H2 样例」 | ✅ 符合 |
| 字面 100% + 真实经营 KPI 已达成 | ❌ 不符合 |
| 完整商业化 SaaS（登录/计费/租户/RBAC 鉴权） | ❌ 不符合 |
| 真实多源 API | ❌ 按约定不做（`application.yml` `jdbc-only`） |

### 9.2 主流程与 API 对照（摘要）

| 能力 | 前端 | 后端 |
|------|------|------|
| 品牌建档 | `/data-prep` + 五维面板 | `POST /api/brand` |
| 信号/看板/workflow | `/radar`、`App` 侧栏 | `?platform=` 全链路 |
| 洞察三图+卡片 | `/insight` | `GET /api/insight/*` |
| TOP50 | `/ranking` | `GET /api/ranking/top50` |
| 机会+报告 | `/opportunity`、`/report` | 详情 + Excel/PDF |
| 应用内 KPI/核心承诺 | `App` / `Insight` / `Mobile` | `/dashboard/product-metrics`、`/core-promise` |
| 多品牌工作区 | 侧栏下拉 | `GET /brand/workspaces` |
| 团队协作 | `/team` | 权限 + 审批 API |
| 迭代2 扩展 | 归因/模型/开放API/供应链/测款/H5 | 对应 Controller 均已实现 |

### 9.3 仍不符合「全部要求」的保留项（已排除真实 API）

1. **真实蝉妈妈/飞瓜/评论 API** — 架构在，默认关闭。  
2. **路线图成功标准字面达成** — 仅有应用内 KPI/核心承诺代理指标。  
3. **完整 SaaS** — 无登录、计费、企业级租户与接口 RBAC。  
4. **24 月趋势时序** — 种子与图表为 12 月。  
5. **评论 NLP** — 情感为关键词规则，非评论挖掘 API。  
6. **跨境全量监控** — 部分平台种子，非 Amazon/Shopee 全链路 API。  
7. **微信正式小程序** — PRD 已改为 H5 专项，无小程序工程。

### 9.4 推荐对外口径

> 功能与 PRD/路线图在**样例数据 + 规则引擎**下已对齐并可演示；**唯一刻意保留的生产缺口是真实商业数据源**；经营 KPI 为**应用内可审计推算**，不可替代外部经营审计。
